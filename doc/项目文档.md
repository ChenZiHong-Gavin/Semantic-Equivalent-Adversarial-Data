...
见[论文理解](Graph-based Fuzz Testing for Deep Learning Inference Engines.md)
...
<img src="https://latex.codecogs.com/png.image?\dpi{110}&space;\hat{a}=\arg&space;\max&space;_{a&space;\in&space;\mathcal{A}}&space;p(a&space;\mid&space;v,&space;q&space;;&space;\theta)" title="\hat{a}=\arg \max _{a \in \mathcal{A}} p(a \mid v, q ; \theta)" />
$$
...
我们的方法优于vanilla训练的基线，早validation，test-dev，test-std数据集上分别取得了1.82%、2.55%和2.6%的收益。
...
3. 扩增数据`image14`和`问题20-1： How many ornamental plants are in this picture?`和`答案: 2`

   ![image14](../fig/image14.png)

4. 扩增数据`imae14-1`和`问题20-1:  How many ornamental plants are in this picture?`和`答案: 2`
